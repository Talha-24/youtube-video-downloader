...
def _run_download_job(job_id, url, quality):
    ...
    try:
        ...
        # Don't assume the exact output filename yt-dlp/ffmpeg produced —
        # look for whatever actually landed in this job's private tmp_dir.
        expected_ext = "mp3" if is_audio_only else "mp4"
        matches = glob.glob(os.path.join(tmp_dir, f"*.{expected_ext}"))
        if not matches:
            raise RuntimeError(
                f"Download finished but no .{expected_ext} file was produced. "
                f"Check that ffmpeg is installed and on PATH."
            )
        final_path = matches[0]

        safe_title = re.sub(r"[^\w\-. ]", "_", info.get("title", "video"))
        download_name = f"{safe_title}.{expected_ext}"
        mimetype = "audio/mpeg" if is_audio_only else "video/mp4"
...
def download_file(job_id):
    with JOBS_LOCK:
        job = JOBS.get(job_id)
        if job is None:
            # Already cleaned up — most likely a duplicate/retry request for
            # a file that was already delivered. Don't error loudly; the
            # first request already handled it.
            return jsonify({"error": "This download has already been delivered or expired."}), 410
        if job.get("status") != "finished":
            return jsonify({"error": "File not ready yet."}), 400
        if job.get("delivered"):
            return jsonify({"error": "This file has already been sent to your browser."}), 410

        # Mark as delivered *before* streaming so a near-simultaneous second
        # request can't slip through and race the cleanup below.
        job["delivered"] = True
        final_path = job["final_path"]
        download_name = job["download_name"]
